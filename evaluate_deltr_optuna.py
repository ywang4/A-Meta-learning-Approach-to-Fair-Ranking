...
class DELTR_Evaluator():
    ...
    def __init__(self, dataset, resultDir, trainingDir, binSize, protAttr):
        self.__trainingDir = trainingDir
    ...
    def evaluate(self):
        if self.__dataset == 'engineering-gender-withoutSemiPrivate':
            self.__trainingDir = self.__trainingDir + 'ENG_GENDER/'
            ...
            result_large = self.__evaluate()

            self.__experimentNamesAndFiles["gamma=large"] = self.__evaluationFilename

        elif self.__dataset == 'compass_race':
            self.__trainingDir = self.__trainingDir + 'COMPASS_RACE/'
        ...
        elif self.__dataset == 'compass_sex':
            self.__trainingDir = self.__trainingDir + 'COMPASS_SEX/'
        ...
        elif self.__dataset == 'german_age25':
            self.__trainingDir = self.__trainingDir + 'GERMAN_AGE25/'
            ...
            self.__experimentNamesAndFiles["gamma=large"] = self.__evaluationFilename
        elif self.__dataset == 'german_age35':
            self.__trainingDir = self.__trainingDir + 'GERMAN_AGE35/'
            ...
            self.__experimentNamesAndFiles["gamma=large"] = self.__evaluationFilename
        elif self.__dataset == 'german_sex':
            self.__trainingDir = self.__trainingDir + 'GERMAN_SEX/'
            ...
            self.__trainingDir = self.__trainingDir + 'ENG_HIGHSCHOOL/'
            # gamma = 'colorblind'
        ...
        ###########################################################################################
            ...
            self.__trainingDir = self.__trainingDir + 'LAW_GENDER/fold_1/'
        ...
        ###########################################################################################
            ...
            self.__trainingDir = self.__trainingDir + 'LAW_RACE/fold_1/'
            # gamma = 'colorblind'
            # pathsForColorblind = [self.__trainingDir + 'GAMMA=0/']
            # pathsToScores = [self.__trainingDir + 'COLORBLIND/']
            #
    ...
    def __evaluate(self, synthetic=False):
        ...
        for name, predGroup in predictedGroups:
            ...
            exposure_prot_pred = self.__calculate_group_exposure(predGroup, origGroup)[0]
            exposure_nprot_pred = self.__calculate_group_exposure(predGroup, origGroup)[1]
            result.loc[i]['query-id'] = name
            result.loc[i]['exposure-prot-pred'] = exposure_prot_pred
            result.loc[i]['exposure-nprot-pred'] = exposure_nprot_pred
            ...
            result.loc[i]['fairness'] = exposure_prot_pred/exposure_nprot_pred
            i += 1
...
#         raise ValueError
#
#     resultDir = './results/mfr/{}/'.format(dataset)
